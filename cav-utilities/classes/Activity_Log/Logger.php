...
class Logger
{
   public function add(
      ...
      $entry['entity_details'] = maybe_unserialize($details);
   ...
   {
      $types = Utils::get_activity_log_default_events();

      if (!in_array($type, array_keys($types))) {
         return new WP_Error('cav_activity_log_type_unknown', __('Event type unknown.', 'cav-utilities'));
      }

      if (191 < strlen($type)) {
         return new WP_Error('cav_activity_log_type_invalid', __('Event type is too long.', 'cav-utilities'));
      }
