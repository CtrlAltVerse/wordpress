...
if (!function_exists('debug')) {
   ...
   function debug()
   {
      ...
      if (
         WP_DEBUG_DISPLAY && !defined('XMLRPC_REQUEST') && !defined('REST_REQUEST') && !defined('MS_FILES_REQUEST') && !(defined('WP_INSTALLING') && WP_INSTALLING) && !wp_doing_ajax() && !wp_is_json_request()
      ) {
         print_r("<pre>{$log}</pre>");
      }
...
if (!function_exists('get_page_component')) {
   ...
   function get_page_component(string $page, array|string $slug, array $args = [], bool $once = false)
   {
      if (str_ends_with($page, '.php')) {
         ...
         preg_match('/pages@([-\w]+)/', $page, $matches);

         if (empty($matches)) {
            throw new \WP_Error('cav_get_page_component_not_in_pages', esc_attr__('Not in pages.', 'cav-utilities'));
         }
         $page = $matches[1];
      }
...
if (!function_exists('render_latte')) {
   ...
   function render_latte(string $template_path, array|object $args = [], bool $echo = true)
   {
      ...
      if (false === $cache_path) {
         throw new \WP_Error('cav_latte_cache_path_not_created', esc_attr__('Directory for Latte Cache could not be created.', 'cav-utilities'));
      }
