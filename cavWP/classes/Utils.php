...
final class Utils
{
   ...
   public static function generate_key(string $key, string $method = 'sanitize', int $length = 172)
   {
      $new_key = match ($method) {
         ...
         default    => throw new \WP_Error('cav_generate_key_method_invalid', sprintf(esc_attr__('%s is not a valid method.', 'cavwp'), $length)),
      };

      if (strlen($new_key) > $length) {
         throw new \WP_Error('cav_generate_key_too_long', sprintf(esc_attr__('Key length must be %s or less.', 'cavwp'), $length));
      }
   ...
   public static function regenerate_image_sizes(int $attachment_id, array $skip_sizes = [])
   {
      ...
      foreach ($target_sizes as $size_name => $size) {
         ...
         if ($compare) {
            if (
               $current_sizes['width']     <= $size['width']
               && $current_sizes['height'] <= $size['height']) {
               continue;
            }

            if (
               isset($current_sizes['sizes'][$size_name])
               && $current_sizes['sizes'][$size_name]['width']  === $size['width']
               && $current_sizes['sizes'][$size_name]['height'] === $size['height']) {
               continue;
            }
         }
